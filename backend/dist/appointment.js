...
router.get("/experts/:expertId/available-slots", async (req, res) => {
    try {
        const { expertId } = req.params;
        const { date } = req.query; // Optional: specific date to check
        console.log(`Getting available slots for expert ID: ${expertId}`);
        let expert = await prisma_1.default.knowledgeProvider.findUnique({
            where: { id: expertId },
            select: {
                id: true,
                isAvailable: true,
        ...
        if (!expert) {
            expert = await prisma_1.default.knowledgeProvider.findUnique({
            ...
            if (expert.availableDays.includes(dayName)) {
                ...
                for (const timeSlot of expert.availableTimes) {
                    ...
                    const isBooked = existingAppointments.some((apt) => {
                        ...
                        // Check if same date
                        if (aptDate.getFullYear() === requestedDate.getFullYear() &&
                ...
                if (expertDaysUpper.includes(dayName)) {
                    for (const timeSlot of expert.availableTimes) {
                        ...
                        const isBooked = existingAppointments.some((apt) => {
                            const aptDate = new Date(apt.appointmentDate);
                            const aptTime = new Date(apt.appointmentTime);
                            if (aptDate.getFullYear() === checkDate.getFullYear() &&
...
router.post("/", auth_1.authenticate, async (req, res) => {
    try {
        const { expertId, appointmentDate, appointmentTime, communicationMedium, questionId, paymentMethod, } = req.body;
        ...
        if (!finalQuestionId) {
            ...
            const placeholderQuestion = await prisma_1.default.questions.create({
                data: {
        ...
        if (expert.user?.email) {
            await (0, notifications_1.sendAppointmentEmail)({
...
router.get("/my-appointments", auth_1.authenticate, async (req, res) => {
    try {
        const seeker = await prisma_1.default.knowledgeSeeker.findUnique({
            where: { userId: req.user.userId },
        });

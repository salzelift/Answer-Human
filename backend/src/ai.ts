...
async function generateWithAI(prompt: string, maxTokens: number = 500): Promise<string> {
...
router.post("/expand-profile", authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
router.post("/enhance-question", authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
router.post("/generate-pitch", authenticate, async (req: AuthRequest, res: Response) => {
  try {

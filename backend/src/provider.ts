...
async function getCategoryIdsWithParents(categoryIdentifiers: string[]): Promise<string[]> {
  ...
  for (const category of categories) {
    ...
    let currentCategoryId: string | null = category.parentCategoryId;
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    // Build where clause
    const where: Prisma.KnowledgeProviderWhereInput = {
      isAvailable: true, // Only show available experts
    };

    // Build array of conditions to combine with AND
    const andConditions: Prisma.KnowledgeProviderWhereInput[] = [];
    ...
    let orderBy: Prisma.KnowledgeProviderOrderByWithRelationInput = {};
    ...
    const experts = await prisma.knowledgeProvider.findMany({
      where,
      orderBy,
      include: {
...
router.get("/:id", async (req: Request, res: Response) => {
  try {
    ...
    const expert = await prisma.knowledgeProvider.findUnique({
      where: { id },
      include: {
        categories: true, // Include categories in response
        user: {
    ...
    if (!expert) {
      ...
      const expertByUserId = await prisma.knowledgeProvider.findUnique({
        where: { userId: id },
        include: {
          categories: true,
          user: {
